...
class SnapperImpl extends Snapper implements Visitor {
	...
	interface PH {
		void pmUp(Numbers x, Numbers y,  int id);
		void pmDrag(Numbers x, Numbers y, int id);
		Model getModel();
 ...
 */
	...
	void pmUp(int x0, int y0, int id, PH ph) {
		...
		ph.pmUp(Numbers.createInteger(x0), Numbers.createInteger(y0), id);
	...
	void pmDrag(int x0, int y0, int id, PH ph) {
		...
		ph.pmDrag(Numbers.createInteger(x0), Numbers.createInteger(y0), id);
