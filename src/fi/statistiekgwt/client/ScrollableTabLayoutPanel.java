 ...
 * https://groups
 * .google.com/forum/?fromgroups=#!topic/google-web-toolkit/wN8lLU23wPA
 */
public class ScrollableTabLayoutPanel extends TabLayoutPanel
{
	...
	public void selectTab(int index, boolean fireEvents)
	{
		...
		else if (getRightPosition(selectedTab) > visibleAreaRightBorder)
		{
			...
			int scrollValue = getRightPosition(selectedTab)
				- visibleAreaRightBorder + halfVisibleAreaWidth - halfTabWidth;
			adjustScroll(-scrollValue);
		}
	}
								...
								.setPopupPositionAndShow(new PopupPanel.PositionCallback()
								{
										...
										int offsetHeight)
									{
										...
										if (offsetHeight > Window
											.getClientHeight())
										{
											// difference between all popup
											// widget height and its child
											// widget height.
											// we need it because when we call
											// popup.setHeight() it actually set
											// height
											// of popup child widget rather than
											// popup itself
											...
											popup.setHeight(Window
												...
												+ "px");
											top = 0;
											// we're about getting the scroll in
											// popup,
											// so increase popup width to not
											// get text wrapped
											popup
												.setWidth((popup.getWidget()
													.getOffsetWidth() + SCROLL_BUFFER)
													+ "px");
											offsetWidth = offsetWidth
												+ SCROLL_BUFFER;
											// if not enough place under the
											// button, lift popup up
										}
										...
										if (offsetWidth > Window
											.getClientWidth())
										{ // hard to imagine but just in case
											int diff = offsetWidth
												- popup.getWidget()
													.getOffsetWidth();
											popup.setWidth(Window
												.getClientWidth() - diff + "px");
											left = 0;
											// if not enough place on the right
											// of the button, shift popup to the
											// left
										}
	...
	private boolean isScrollingNecessary()
	{
		...
		isScrollingNecessary = lastTab != null && getRightPosition(lastTab) > getTabBarWidth();
		
		return isScrollingNecessary;
	}
	...
	{
		int position = widget.getElement().getOffsetLeft(); 
		return position;
	}
	...
	{
//		int width = tabBar.getElement().getParentElement().getClientWidth(); // is 0!
//		int width = tabBar.getOffsetWidth(); // is totale browserbreedte
//		int width = tabBar.getElement().getClientWidth(); // is totale browserbreedte
//		return width; // corrigeert niet voor de breedte van de scrollbuttons
		return width - resources.back().getWidth() - resources.next().getWidth() - resources.menu().getWidth();
	}

	private int getScrollPanelWidth()
	{
		int width = scrollPanel.getElement().getParentElement().getClientWidth(); 
		return width;
	}
	...
	public static interface Resources extends ClientBundle
	{
		...
		@Source("fi/statistiekgwt/client/images/arrow-216-16down.gif")
		ImageResource menu();
	}
}
