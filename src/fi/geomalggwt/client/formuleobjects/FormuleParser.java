...
*/	
	...
	{	
		...
		index = 0;
		while(index >-1)
		{	index = s.indexOf("s*g*f*");
			if(index >-1)s = s.substring(0,index) + "sgf" + s.substring(index+6);
		}
		index = 0;
		while(index >-1)
		...
		index = 0;
		while(index >-1)
		{	index = s.indexOf("p*o*i*s*s*o*n*c*d*f*");
			if(index >-1)s = s.substring(0,index) + "poissoncdf" + s.substring(index+20);
		}
		index = 0;
		while(index >-1)
		{	index = s.indexOf("p*o*i*s*s*o*n*p*d*f*");
			if(index >-1)s = s.substring(0,index) + "poissonpdf" + s.substring(index+20);
		}
		index = 0;
		while(index >-1)
...
/*            
		...
		{	String string = s.substring(11,s.length()-1);
			...
			if(e1==null || e2==null) return null;
			else return new PoissonCDF(e1,e2);
		}
*/
/*            
		...
		{	String string = s.substring(11,s.length()-1);
			...
			Expressie e2 = parse(string.substring(lastIndex+1));
			if(e1==null || e2==null) return null;
			else return new PoissonPDF(e1,e2);
		}
*/
/*            
...
/*            
		...
		{	String string = s.substring(4,s.length()-1);
			...
			{	int lastIndex = string.lastIndexOf('_');
				...
				Expressie e2 = parse(string.substring(lastIndex+1));
				if(e1==null || e2==null)return null;
				else return new SigRoundStandard(e1,e2);
			}
