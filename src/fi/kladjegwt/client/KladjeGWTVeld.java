...
public class KladjeGWTVeld 
{
	...
	{	
		...
		ImageResource goedvinkResource = eigenaar.kladjeGWTClientBundle.goedvinkResource();
		goedvinkImage = new Image(goedvinkResource);
		goedvinkImageElement = ImageElement.as(goedvinkImage.getElement());
		
		ImageResource halfvinkResource = eigenaar.kladjeGWTClientBundle.halfvinkResource();
		halfvinkImage = new Image(halfvinkResource);
		halfvinkImageElement = ImageElement.as(halfvinkImage.getElement());
		
		ImageResource foutkruisResource = eigenaar.kladjeGWTClientBundle.foutkruisResource();
...
//			else 
	...
	public HashMap<String,Object> getState(boolean end)
	{
		...
		if(end && mouseMode!=ivmOptie && currentHiddenStrokeContainer!=null) {
...
//		h.put("currentSCIndex", new Integer(currentSCIndex));
	...
	public void setState(Map<String, Object> map, boolean init)
	{
		...
		{	
			...
			//sc.scale(1.0/schrijfLeesFactor);
			//hiddenStrokeContainers.set(sCnt,sc);
		}
	...
	public void mouseDownTouchStartAction(int eventX, int eventY)
	{
		...
		else if (mouseMode == formuleOptie)
		{
			...
			{	
				...
				if(currentHiddenStrokeContainer.getCheckButtonArea().contains(eventX, eventY) && currentHiddenStrokeContainer.isActive()) 
				{
					eigenaar.sendEquation(activeHSCNumber);
					closeCurrentHiddenContainer();
					paint();
				...
				if(currentHiddenStrokeContainer.getRecognizeButtonArea().contains(eventX, eventY) && currentHiddenStrokeContainer.isActive()) {
					//currentHiddenStrokeContainer.setRecognizeOff(true);
					return;
				}
				if(currentHiddenStrokeContainer.getApproxButtonArea().contains(eventX, eventY) && currentHiddenStrokeContainer.isActive()) {
			...
			if(currentStrokeContainer!=null && currentStrokeContainer.getNotRecognizeButtonArea().contains(eventX, eventY) 
					&& !(currentStrokeContainer.isCorrect() || currentStrokeContainer.isFalse() || currentStrokeContainer.isHalf())) {
			...
			if(currentStrokeContainer!=null && currentStrokeContainer.getEraserActive() && currentStrokeContainer.writeBoxContains(eventX, eventY)) {
...
//				lastStroke.scale(0, 0, 10);
				...
				if(stroke.getParsePointsbox().height>160 || currentStrokeContainer.getStrokeCount()==1 && stroke.getParsePointsbox().width>200)
					currentStrokeContainer.setRecognizeOff(true);
