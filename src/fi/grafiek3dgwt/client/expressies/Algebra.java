...
{	
	...
	public static boolean checkGelijkwaardigComplex(Expressie e1, Expressie e2, String[] varNamen,double[] tryValues)
	{	double[] defaultTryIntValues = {0.101, 1.102, 2.103, 3.104, 7.105};
		if(tryValues==null) tryValues = defaultTryIntValues;
		Complex e1Waarde = e1.geefWaardeComplex();
		Complex e2Waarde = e2.geefWaardeComplex();
		if(varNamen.length==0  || (e1Waarde!=null && e2Waarde!=null))
		{	boolean nan1 = (Complex.isInfinite (e1Waarde) || e1Waarde==null);
			boolean nan2 = (Complex.isInfinite (e2Waarde) || e1Waarde==null);
			boolean ongelijk = !Complex.isEqualDiff(e1Waarde, e2Waarde, 0.000000001) && !Complex.isEqualWithinLimits(e1Waarde, e2Waarde, 0.000000001);
			//System.out.println(e1.toString() +"  "+ e2.toString());
			//System.out.println(FormuleParser.formuleString(e1.toString()) +"  "+ FormuleParser.formuleString(e2.toString()));
			//System.out.println(e1Waarde.toString() +"  "+ e2Waarde.toString());
			if(nan1 && !nan2 || !nan1 &&nan2 || (ongelijk && !(nan1 && nan2)))
			{	return false;
			}
			return true;
		...
		{	String[] varNamenNieuw = new String[varNamen.length-1];
			...
			{	double value = tryValues[i];
                ...
                double[] tryValuesNieuw = new double[tryValues.length];
                for(int j=0 ; j<tryValues.length ; j++) tryValuesNieuw[j] = tryValues[j]+0.012*tryValuesWidthfactor;
                boolean gelijkwaardig = checkGelijkwaardigComplex(ee1,ee2,varNamenNieuw,tryValuesNieuw);
				if(!gelijkwaardig)return false;
			}
			return true;
		}
	...
	public static boolean gelijkGevormd(Vergelijking v1, Vergelijking v2)
	{	if(v1==null || v2==null)return false;
		return (gelijkGevormd(v1.kind1, v2.kind1) && gelijkGevormd(v1.kind2, v2.kind2) && v1.geefVergTeken().equals(v2.geefVergTeken())
		|| gelijkGevormd(v1.kind1, v2.kind2) && gelijkGevormd(v1.kind2, v2.kind1) && v1.geefVergTeken().equals(Vergelijking.geefInvVergTeken(v2.geefVergTeken())));
	}
	
	public static boolean gelijkGevormd(VergelijkingMeerv vm1, VergelijkingMeerv vm2)
	{	if(vm1==null || vm2==null) return false;
		if(vm1.vergelijkingen.length != vm2.vergelijkingen.length) return false;
		
		
		int aantal = vm1.vergelijkingen.length;
		boolean[][] zijnGelijk = new boolean[aantal][aantal];
		for(int i=0 ; i<vm1.vergelijkingen.length ; i++)
		{	for(int j=0 ; j<vm1.vergelijkingen.length ; j++)
			{	zijnGelijk[i][j] = gelijkGevormd(vm1.vergelijkingen[i], vm2.vergelijkingen[j]);
			}
		}
		/*
		boolean gelijk = true;
		...
		{	boolean fout;
			boolean start=false;
			for(int j=0 ; j<vm1.vergelijkingen.length && j!=i; j++)
			{	start = i==0 && j==1;
				fout = !zijnGelijk[i][i] || !zijnGelijk[j][j] && !zijnGelijk[i][j] || !zijnGelijk[j][i];	
		...
		*/
		if(aantal==2)
		{
			if((!zijnGelijk[0][0] || !zijnGelijk[1][1]) && (!zijnGelijk[0][1] || !zijnGelijk[1][0]))
			{	return false;
			}
			else
			{	return true;
		...
		else
		{
			for(int i=0 ; i<vm1.vergelijkingen.length ; i++)
			{
				if(!gelijkGevormd(vm1.vergelijkingen[i], vm2.vergelijkingen[i])) return false;
			}
			return true;
		}
	}
	...
	{	/*if(e1==null || e2==null)return false;
		if(e2 instanceof BasisExpressie && e2.toString().equals("Q")) return true;
		if(e1 instanceof BasisExpressie && e2 instanceof BasisExpressie && e1.toString().equals(e2.toString())) return true;
		if(e1.getClass().getName().equals(e2.getClass().getName()))
		{	if(e1.kind2==null && e2.kind2==null) return gelijkGevormd(e1.kind1, e2.kind1);
			else if(e1.kind3==null && e2.kind3==null) return gelijkGevormd(e1.kind1, e2.kind1)&& gelijkGevormd(e1.kind2, e2.kind2);
			else if(e1.kind4==null && e2.kind4==null) return gelijkGevormd(e1.kind1, e2.kind1)&& gelijkGevormd(e1.kind2, e2.kind2) && gelijkGevormd(e1.kind3, e2.kind3);
	...
	{	if(e1==null || e2==null) return false;
		...
		if( vorm && e2 instanceof BasisExpressie && e2.toString().equals("Q")) return true;
		...
		//else if(e1 instanceof Abs && e2 instanceof Abs)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
		//}
		//else if(e1 instanceof Abs || e2 instanceof Abs)
		//{	return false;
		//}
		//else if(e1 instanceof Conjug && e2 instanceof Conjug)
        //{   return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
        //}
        //else if(e1 instanceof Conjug || e2 instanceof Conjug)
        //{   return false;
        //}
		//else if(e1 instanceof Faculteit && e2 instanceof Faculteit)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
		//}
		//else if(e1 instanceof Faculteit || e2 instanceof Faculteit)
		//{	return false;
		//}
		else if(e1 instanceof Bin && e2 instanceof Bin)
		{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
		}
		else if(e1 instanceof Bin || e2 instanceof Bin)
		{	return false;
		}
		//else if(e1 instanceof Diff && e2 instanceof Diff)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
		//}
		//else if(e1 instanceof Diff || e2 instanceof Diff)
		//{	return false;
		//}
		//else if(e1 instanceof DiffPartial && e2 instanceof DiffPartial)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm);
		//}
		//else if(e1 instanceof DiffPartial || e2 instanceof DiffPartial)
		//{	return false;
		//}
		//else if(e1 instanceof Prv && e2 instanceof Prv)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm) && zijnGelijk(e1.kind4,e2.kind4,vorm);
		//}
		//else if(e1 instanceof Prv || e2 instanceof Prv)
		//{	return false;
		//}
		//else if(e1 instanceof Sigma && e2 instanceof Sigma)
        //{   return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm) && zijnGelijk(e1.kind4,e2.kind4,vorm);
        //}
        //else if(e1 instanceof Sigma || e2 instanceof Sigma)
        //{   return false;
		...
		//{	return false;
		//}
		//else if(e1 instanceof NormalCDF && e2 instanceof NormalCDF)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm) && zijnGelijk(e1.kind4,e2.kind4,vorm);
		//}
		//else if(e1 instanceof NormalCDF || e2 instanceof NormalCDF)
		//{	return false;
		//}
		//else if(e1 instanceof InvNorm && e2 instanceof InvNorm)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm);
		//}
		//else if(e1 instanceof InvNorm || e2 instanceof InvNorm)
		//{	return false;
		//}
		//else if(e1 instanceof BinomCDF && e2 instanceof BinomCDF)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm);
		//}
		//else if(e1 instanceof BinomCDF || e2 instanceof BinomCDF)
		//{	return false;
		//}
		//else if(e1 instanceof BinomPDF && e2 instanceof BinomPDF)
		//{	return zijnGelijk(e1.kind1,e2.kind1,vorm) && zijnGelijk(e1.kind2,e2.kind2,vorm) && zijnGelijk(e1.kind3,e2.kind3,vorm);
		//}
		//else if(e1 instanceof BinomPDF || e2 instanceof BinomPDF)
		//{	return false;
	...
	{	if(e instanceof Optelling
		...
		else if(e instanceof Aftrekking && e.kind1.isBasis && e.kind1.geefWaarde()==0)
		{	if(e.kind2 instanceof Optelling
					&& e.kind2.kind1 instanceof BasisExpressie
				   && !Double.isNaN(e.kind2.kind1.geefWaarde())
				   && e.kind2.kind2 instanceof Deling
				   && e.kind2.kind2.kind1 instanceof BasisExpressie
				   && !Double.isNaN(e.kind2.kind2.kind1.geefWaarde())
				   && e.kind2.kind2.kind2 instanceof BasisExpressie
				   && !Double.isNaN(e.kind2.kind2.kind2.geefWaarde())
					)
			{	return true;
			}
			else return false;
	...
	{	if(e instanceof Wortel)return new Wortel(herleid(e.kind1));
		...
		if(e instanceof ArcTangens)return new ArcTangens(herleid(e.kind1));
		...
		if(e instanceof SigRound)return new SigRound(herleid(e.kind1),herleid(e.kind2),herleid(e.kind2));
	...
	{	if(e.kind1!=null)e.kind1 = herleidMild(e.kind1, breukenGemengd);
		...
		if(e.isWaarde())
		{	e = evalueerGetalsExpressie(e, breukenGemengd);
			
			//Vector v = Algebra.geefTermen(e,new Vector());
			//v = sorteerTermen(v);
			//e = Algebra.maakTermenExpressie(v);
		}
...
//        }
	...
	{	if(exp instanceof DecRound)
		{	
			/*if("MW".equals(WiskOpdr.deployVariant) && exp.kind2.geefWaarde()>0.000000001)
			{	String formatString = "0.";
				...
				return new BasisExpressie(df.format(exp.geefWaarde()));
			}
			else*/ 
...
*/		
		...
		{	
			...
			{	String formatString = "0.";
				...
				else return new BasisExpressie(exp.geefWaarde());
			}
			else 
			return new BasisExpressie(exp.geefWaarde());
		}
