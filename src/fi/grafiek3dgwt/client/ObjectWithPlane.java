...
{
	public static CssColor objectColor = Grafiek3DComponent.graphColor;
    ...
    {   
    	objectColor = Grafiek3DComponent.objectColor;
...
//System.out.println("right");                                
                ...
                {   
                    ...
                    {   int index = replacement.numVertices;
                        ...
                        inds[0] = index;
                        inds[1] = index + 1;
                        Facet3D lFacet = new Facet3D(replacement.vertices, inds, Grafiek3DComponent.black);
...
//System.out.println("touches from left");                                    
                ...
                {   
                    ...
                    {   int index = replacement.numVertices;
                        ...
                        inds[0] = index;
                        inds[1] = index + 1;
                        Facet3D rFacet = new Facet3D(replacement.vertices, inds, Grafiek3DComponent.black);
        ...
        {   Facet3D repFacet = replacement.facets[cnt];
            ...
            {   int index = Facet3D.containsVertex(
                ...
                {    if (!isOn)
                     ...
                     {   if ((facet.vertexCodes[index] % HIDDENSHIFT) ==
                    	 lineColorIndex)
                             repFacet.vertexCodes[pCnt] = facet.vertexCodes[index];
                         else    
...
//System.out.println("index < 0");                                                
                    ...
                    if (repFacet.numPoints == 2)
                        repFacet.vertexCodes[pCnt] = lineColorIndex +
                                                     HIDDENSHIFT;
                    else
                    {   
...
//System.out.println("eIndex >= 0");                            
                            if (facet.edgeCodes[eIndex] == lineColorIndex)
                                repFacet.vertexCodes[pCnt] = lineColorIndex;
                            else    
                                repFacet.vertexCodes[pCnt] = planeColorIndex;
                        }    
...
//indexCnt++;
                ...
                {   Vector construction = new Vector();
                    ...
                    if (isOnLine)                    
                        repFacet.edgeCodes[pCnt] = lineColorIndex;                    
                    else
                    ...
                    if (index < 0) // forced hidden
                    {
                        ...
                        if (isOnLine)
                            replacement.facets[i].edgeCodes[j] = 
                            	lineColorIndex + HIDDENSHIFT;
                        else
                            replacement.facets[i].edgeCodes[j] = 
                                planeColorIndex + HIDDENSHIFT;
                    }        
...
//System.out.println("indCnt = " + indCnt);                
                ...
                Facet3D cutFacet = new Facet3D(cut.vertices, 
                    finalCutInds, planeColor);
                Facet3D.copyAttributes(leftFacet, cutFacet, false);
                // override!!
                cutFacet.color = planeColor;
...
//System.out.println("before recut facets = " + cut.numFacets);            
                        ...
                        cut = cutWithLine(cut, li, lineColorIndex);
...
//System.out.println("inPlane");                    
                    ...
                    {    if (cutFacet.edgeCodes[m] < 0)
                            cutFacet.edgeCodes[m] = planeOutlineColorIndex + 40;
...
*/                    
                ...
                {   // facet is the cut, add to right
                    ...
                    for (int k = 0; k < owp.facets[i].numPoints; k++)
                        inds[k] = k + firstIndex;
                    Facet3D leftCutFacet = new Facet3D(left.vertices, inds, objectColor);
                    left.addFacet(leftCutFacet);
                    Facet3D.copyAttributes(owp.facets[i], leftCutFacet, false);
                    leftCutFacet.color = objectColor;
        ...
        {   Object ob = recipe.elementAt(i);
            ...
            {   Line3D li = (Line3D) ob;
                ObjectGroup3D tempStartGroup = new ObjectWithLine(startGroup, 
                    li.point1, li.point2, lineColorIndex, llFactor);    
                // replacement made    
                if (tempStartGroup.objects.size() > 1)    
                {   startGroup = tempStartGroup;
                    startGroup.fixFacetArray(); //!!!    
