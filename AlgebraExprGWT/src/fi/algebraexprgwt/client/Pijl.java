...
public class Pijl
{
	...
	private static final CssColor GRAY = CssColor.make(128, 128, 128);
		...
		if (dx != 0 || dy != 0)
		{
			...
			if ((dx < 2 * rmax && dx > -2 * rmax) && dy >= 0)
			{
				...
				if (color.toString().equals(BLACK.toString()))
				{
					...
					// gIm.drawArc(xb0, yb0, 2*rmax, 2*rmax, 90+teken*90,
					// teken*(booghoek+2));
			...
			else if (dy < 2 * rmax && dy >= 0)
			{
				...
				if (color.toString().equals(BLACK.toString()))
				{
					...
					gIm.beginPath();
					gIm.arc(xb0 + teken * (int) b - 2 * teken * rmax + rmax, yb1 + rmax, rmax, startAngle, endAngle,
						antiClockWise);
					gIm.stroke();
			...
			{
				if (Math.abs(dx / 4) < rmax)
					rmax = Math.abs((int) dx / 4);
				xb0 = x0 - rmax + teken * rmax;
				yb0 = y0 - rmax;
				xb1 = x1 - rmax - teken * rmax;
				yb1 = y1 - rmax;
