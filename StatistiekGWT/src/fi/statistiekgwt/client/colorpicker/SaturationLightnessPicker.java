...
public class SaturationLightnessPicker extends Composite
{
	...
	public HandlerRegistration addColorChangedHandler(
		IColorChangedHandler handler)
	{
	...
	private void fireColorChanged(CssColor color)
	{
		fireEvent(new ColorChangedEvent(color));
	}
	...
	public void setColor(String color)
	{
		...
		hue = hsl[0];
		handleX = (int) Math.min(Math.max(Math.round(hsl[1] * 180d / 100d), 0),
			179);
		handleY = (int) Math.min(Math.max(Math.round(hsl[2] * 180d / 100d), 0),
			179);
		drawGradient(true);
		fireColorChanged(color);
	}
